...
### 1. **支付与凭证上传（`src/pages/order/payment.tsx`）**

- 使用 `Taro.chooseImage` + `orderService.uploadPaymentProof`
- 成功后跳转订单详情或支付成功页
...
const uploadProof = async (orderId: string) => {
  ...
  await orderService.uploadPaymentProof(orderId, tempFilePath);
...
### 3. **虚拟购物卡赠礼（`src/pages/shopping-card/share.tsx`）**
...
### 4. **虚拟购物卡领取（`src/pages/shopping-card/claim.tsx`）**
