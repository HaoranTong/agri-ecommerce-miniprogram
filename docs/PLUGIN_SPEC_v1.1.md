...
- 所有控制器返回 `WP_REST_Response` 或 `WP_Error`。错误码、HTTP 状态需与《API_CONTRACT_V2.3》完全一致。
- 常用错误枚举：

| error_code                    | HTTP 状态 | 典型触发点                                         |
| ----------------------------- | --------- | -------------------------------------------------- |
| `invalid_token`               | 401       | JWT 校验失败                                       |
| `invalid_card`                | 400       | 购物卡 PIN 错误                                    |
| `card_already_bound`          | 409       | 购物卡已被他人绑定                                 |
| `card_locked`                 | 423       | 连续输错 PIN 导致锁定                              |
| `share_token_invalid`         | 410       | 分享链接过期或被关闭                               |
| `share_token_active`          | 409       | 重新生成分享包前旧链接仍有效                       |
| `pin_reveal_limit_reached`    | 423       | 超出 PIN 查看次数                                  |
| `points_not_enough`           | 409       | 积分预占超出用户可用额度                           |
| `points_action_invalid`       | 400       | `POST /points/redeem` 的 action 非合法值           |
| `points_reservation_not_found`| 404       | `confirm/release` 时找不到预占记录                |
| `poster_not_found`            | 404       | 请求不存在的营销海报模板                           |
| `not_authorized_agent`        | 403       | 非代理商调用代理接口                               |

```php
return new WP_Error(
  'points_not_enough',
  __( '可用积分不足', 'myshop-core' ),
  [ 'status' => 409 ]
);
```
...
**文档版本**：V1.1（插件开发冻结基线）  
**最后更新**：2025年11月22日  
**输出格式**：Markdown（`docs/PLUGIN_SPEC.md`）
