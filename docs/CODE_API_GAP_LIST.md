...
> 扫描范围：points、orders、user、gift card 相关页面及 `services/api.ts`。以下列出与《API_CONTRACT_V2.3.md》不一致的字段/交互，供后续排期修复。
...
> 后续：以上差异将作为“记录需调整的字段”基线，并同步到 `docs/API_CONTRACT_V2.3.md` 或对应模块的实现计划中。若扫描其他页面发现新的字段差异，可继续在本文件附加表格。