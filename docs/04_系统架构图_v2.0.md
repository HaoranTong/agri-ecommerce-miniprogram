...
- **职责**：
  ...
  - 调用 `/wp-json/myshop/v1/...` 统一接口（封装在 `src/services/api.ts`）
...
sequenceDiagram
    ...
    Buyer->>API: POST /gift-cards/share
    API->>DB: 生成 share_token + 记录 share_logs
    API-->>Buyer: 返回分享口令/二维码路径
    Recipient->>API: POST /gift-cards/share/{token}/claim
    API->>DB: 校验 token & 绑定 redeemer_id
    API-->>Recipient: 返回购物卡信息
    API->>DB: 更新 share_logs 状态
```
