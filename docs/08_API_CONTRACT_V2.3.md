...
- ✅ **V2.3.3更新**：登录返回新用户/资料完整度标记；资料完善改为用户在个人中心自助完成（提现等强实名场景再校验）
...
> 3. `POST /gift-cards/share` - 分享前不强制校验实名信息（仅在提现/强实名场景校验）
>
...
> **V2.3.2 更新**：新增 `avatar`（微信头像URL）和 `gender`（0=未知/1=男/2=女）字段。前端可在个人中心上传头像昵称，登录阶段不强制采集。
> 
...
> ⚠️ 前端仅在提现/强实名场景校验真实姓名与手机号；未完善时引导至资料完善页。
...
**用途**：积分兑换佣金提现申请
> **前置条件**：需完成实名认证（真实姓名 + 已验证手机号），否则返回 `verification_required`（403）。
  ...
  "data": [
    {
      ...
      "title": "完善个人资料",
      "description": "补齐昵称、真实姓名、手机号（强实名场景）",
      "reward_points": 30,
      "status": "available",
      "progress": 1,
      "goal": 1,
